...
def main():
    ...
    parser.add_argument("--in_root", default=DEFAULT_IN_ROOT, help="Input directory (e.g., data/mini_vox)")
    parser.add_argument("--out_root", default=DEFAULT_OUT_ROOT, help="Output directory (e.g., data/processed)")
    parser.add_argument("--split_valid", action="store_true", default=True, help="Split valid set into valid/test")
    ...
    # 1. Collect files
    train_files = list((in_root / "train").glob("*.wav")) if (in_root / "train").exists() else []
    valid_files = list((in_root / "valid").glob("*.wav")) if (in_root / "valid").exists() else []
    
    # If no train/valid folders found, try searching recursively in root
    if not train_files and not valid_files:
        print("No train/valid folders found. Searching recursively...")
        all_files = list(in_root.rglob("*.wav"))
        # Assume everything is train if structure is unknown, or try to parse
        train_files = all_files
        valid_files = []

    print(f"Found {len(train_files)} train files, {len(valid_files)} valid files.")

    # 2. Prepare Valid/Test Split
    valid_speakers = set()
    test_speakers = set()
    
    if args.split_valid and valid_files:
