...
def verify_interactive(duration=3, model_path=None, threshold=0.75):
    ...
    record_once(out, duration=duration)
    model_verify(out, model_path=model_path or os.path.join(MODELS_DIR, "ecapa_best.pth"), threshold=threshold)


def verify_by_file(wav_path, model_path=None, threshold=0.75):
    model_verify(wav_path, model_path=model_path or os.path.join(MODELS_DIR, "ecapa_best.pth"), threshold=threshold)
...
def main():
    ...
    if args.cmd == "enroll":
        ...
        if args.wav:
            ...
            verify_interactive(model_path=args.model_path, threshold=args.threshold)
    else:
        parser.print_help()
